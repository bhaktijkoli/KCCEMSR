@extends('layouts.master')
@section('pre')
	@php
	$title = "Committees";
	$menu_item = 'aboutus';
@endphp
@endsection
@section('content')
	<div class="section clearfix object-non-visible" data-animation-effect="fadeIn">
		<div class="container main-content-sub">
			<div class="row">
				<h1 id="about" class="title text-center"><span>Committees</span></h1>
				<div class="space"></div>
			</div>
			{{-- 1. Internal Quality Assurance Cell & Accreditation Working Committee --}}
			<div class="row">
				<div class="col-sm-12">
					<h2>1. Internal Quality Assurance Cell & Accreditation Working Committee</h2>
					<table class="table table-responsive table-hover table-striped">
						<thead>
							<tr>
								<th width="10%">Sr No</th>
								<th width="60%">Name</th>
								<th width="30%">Designation</th>
							</tr>
						</thead>
						<tbody>
							<tr>
								<td>1</td>
								<td>Dr. Vilas Nitnaware</td>
								<td>Chairman</td>
							</tr>
							<tr>
								<td>2</td>
								<td>Dr. Arundhati Chakrabarti</td>
								<td>IQAC Coordinator</td>
							</tr>
							<tr>
								<td>3</td>
								<td>Prof.Rajiv Iyer</td>
								<td>Member</td>
							</tr>
							<tr>
								<td>4</td>
								<td>Dr.Shelley Oberoi</td>
								<td>Member</td>
							</tr>
							<tr>
								<td>5</td>
								<td>Prof.Amarja Adgaonkar</td>
								<td>Member</td>
							</tr>
							<tr>
								<td>6</td>
								<td>Dr.Baban Rindhe</td>
								<td>Member</td>
							</tr>

							<tr>
								<td>7</td>
								<td>Prof.Shirkesh Poojari</td>
								<td>Member</td>
							</tr>
							<tr>
								<td>8</td>
								<td>Prof.Vaishali Bhalerao</td>
								<td>Member</td>
							</tr>
							<tr>
								<td>9</td>
								<td>Prof.Ashwini Jayawant</td>
								<td>Member</td>
							</tr>
							<tr>
								<td>10</td>
								<td>Prof.Varsha Wangikar</td>
								<td>Member</td>
							</tr>
							<tr>
								<td>11</td>
								<td>A.P. Mandar Ganjapurkar</td>
								<td>Member</td>
							</tr>
							<tr>
								<td>12</td>
								<td>Prof Sonal Balpande</td>
								<td>Member</td>
							</tr>
							<tr>
								<td>13</td>
								<td>Mr. Mahesh Dalvi</td>
								<td>Office Supritendent</td>
							</tr>
							<tr>
								<td>14</td>
								<td>Ms. Nayna Khandekar</td>
								<td>Lab Assistant</td>
							</tr>
							<tr>
								<td>15</td>
								<td>Mr. Anil Mourya</td>
								<td>Lab Assistant</td>
							</tr>
							<tr>
								<td>16</td>
								<td>Dr. Nitin Kale</td>
								<td>External Advisor</td>
							</tr>
							<tr>
								<td>17</td>
								<td>Mrs. Rashmi Joshi</td>
								<td>NGO</td>
							</tr>
							<tr>
								<td>18</td>
								<td>Mr. Amanjeet Malhotra</td>
								<td>Student Representative </td>
							</tr>
							<tr>
								<td>19</td>
								<td>Mr. Harshad Bhoir</td>
								<td>Alumni</td>
							</tr>
						</tbody>
					</table>
				</div>
			</div>
			<div class="row">
				<div class="col-sm-12">
					<h2>2. Anti Ragging Commiittee & Anti Ragging Squad</h2>
					<table class="table table-responsive table-hover table-striped">
						<thead>
							<tr>
								<th width="10%">Sr No</th>
								<th width="60%">Name</th>
								<th width="30%">Designation</th>
							</tr>
						</thead>
						<tbody>
							<tr>
								<td>1</td>
								<td>Dr. Vilas Nitnaware</td>
								<td>Chairman</td>
							</tr>
							<tr>
								<td>2</td>
								<td>Prof. Reeta Shaktivel</td>
								<td>Member</td>
							</tr>
							<tr>
								<td>3</td>
								<td>Ad. Rahul Ranpise</td>
								<td>Member</td>
							</tr>
							<tr>
								<td>4</td>
								<td>A.P. Asmita Deshmukh</td>
								<td>Member</td>
							</tr>
							<tr>
								<td>5</td>
								<td>A.P. Sandesh Bhingardeve</td>
								<td>Member</td>
							</tr>
							<tr>
								<td>6</td>
								<td>A.P. Hasib Shaikh</td>
								<td>Member</td>
							</tr>
							<tr>
								<td>7</td>
								<td>Mr. Mahesh Dalvi</td>
								<td>Faculty representative</td>
							</tr>
							<tr>
								<td>8</td>
								<td>Prof. Sandesh Bhingardeve</td>
								<td>Faculty representative</td>
							</tr>
							<tr>
								<td>9</td>
								<td>Mr.Sandeep Parte</td>
								<td>parent representative</td>
							</tr>
							<tr>
								<td>10</td>
								<td>Prof. Hasib Shaikh</td>
								<td>Faculty representative</td>
							</tr>
							<tr>
								<td>11</td>
								<td>Mr. Mahesh Dalvi</td>
								<td>Non Teaching Member</td>
							</tr>
							<tr>
								<td>12</td>
								<td>Ms. Parita Danole</td>
								<td>Senior Student representative</td>
							</tr>
							<tr>
								<td>13</td>
								<td>Ms. Rashi Shinde</td>
								<td>Fresher Student representative</td>
							</tr>
						</tbody>
					</table>
				</div>
			</div>
			{{-- 3. Library Committee --}}
			<div class="row">
				<div class="col-sm-12">
					<h2>3. Library Committee</h2>
					<table class="table table-responsive table-hover table-striped">
						<thead>
							<tr>
								<th width="10%">Sr No</th>
								<th width="60%">Name</th>
								<th width="30%">Designation</th>
							</tr>
						</thead>
						<tbody>
							<tr>
								<td>1</td>
								<td>Dr. Vilas Nitnaware</td>
								<td>Chairman</td>
							</tr>
							<tr>
								<td>2</td>
								<td>Mr. Prashant Sutone</td>
								<td>Secretary</td>
							</tr>
							<tr>
								<td>3</td>
								<td>A.P. Pragati C</td>
								<td>Member</td>
							</tr>
							<tr>
								<td>4</td>
								<td>Prof. Vaishali Bhalerao</td>
								<td>Member</td>
							</tr>
							<tr>
								<td>5</td>
								<td>Prof. Swati Panwalkar</td>
								<td>Member</td>
							</tr>
							<tr>
								<td>6</td>
								<td>Prof. Sulochana Madchane</td>
								<td>Member</td>
							</tr>
							<tr>
								<td>7</td>
								<td>Prof. Shailja Mohite</td>
								<td>Member</td>
							</tr>
							<tr>
								<td>8</td>
								<td>Prof. Tushar Dangat</td>
								<td>Member</td>
							</tr>
							<tr>
								<td>9</td>
								<td>Prof. Shwan Langford</td>
								<td>Member</td>
							</tr>
						</tbody>
					</table>
				</div>
			</div>
			{{-- 4. Student Council Committee --}}
			<div class="row">
				<div class="col-sm-12">
					<h2>4. Student Council Committee</h2>
					<table class="table table-responsive table-hover table-striped">
						<thead>
							<tr>
								<th width="10%">Sr No</th>
								<th width="60%">Name</th>
								<th width="30%">Designation</th>
							</tr>
						</thead>
						<tbody>
							<tr>
								<td>1</td>
								<td>Prof. Reeta Shaktivel</td>
								<td>Chairman</td>
							</tr>
							<tr>
								<td>2</td>
								<td>Prof. Hasib Shaikh</td>
								<td>Member</td>
							</tr>
							<tr>
								<td>3</td>
								<td>A.P. Pragati C.</td>
								<td>Member</td>
							</tr>
							<tr>
								<td>4</td>
								<td>Mr. Harsh Kanzariya</td>
								<td>Joint Secretary</td>
							</tr>
							<tr>
								<td>5</td>
								<td>Mr. Amanjeet Singh Malhotra</td>
								<td>Cultural Secretary</td>
							</tr>
							<tr>
								<td>6</td>
								<td>Student Representative</td>
								<td>Technical Secretary</td>
							</tr>
							<tr>
								<td>7</td>
								<td>Mr. Jaison Thomas</td>
								<td>Sports Secretary</td>
							</tr>
						</tbody>
					</table>
				</div>
			</div>
			{{-- 5. Research & Development Committee --}}
			<div class="row">
				<div class="col-sm-12">
					<h2>5. Research & Development Committee</h2>
					<table class="table table-responsive table-hover table-striped">
						<thead>
							<tr>
								<th width="10%">Sr No</th>
								<th width="60%">Name</th>
								<th width="30%">Designation</th>
							</tr>
						</thead>
						<tbody>
							<tr>
								<td>1</td>
								<td>Dr. Vilas Nitnaware</td>
								<td>Chairman</td>
							</tr>
							<tr>
								<td>2</td>
								<td>Dr. N.S. Poonawala</td>
								<td>Dean</td>
							</tr>

							<tr>
								<td>3</td>
								<td>Dr. Baban Rindhe</td>
								<td>Coordinator</td>
							</tr>
							<tr>
								<td>4</td>
								<td>Dr. Shelley Oberoi</td>
								<td>Member</td>
							</tr>
							<tr>
								<td>5</td>
								<td>Dr. Arundhati Chakrabarti</td>
								<td>Member</td>
							</tr>
							<tr>
								<td>4</td>
								<td>A.P. Rajiv Iyer</td>
								<td>Member</td>
							</tr>
							<tr>
								<td>6</td>
								<td>A.P. Mandar Ganjapurkar</td>
								<td>Member</td>
							</tr>
							<tr>
								<td>7</td>
								<td>Prof. Aarti Bakshi</td>
								<td>Member</td>
							</tr>
							<tr>
								<td>8</td>
								<td>A.P. Amarja Adgaonkar</td>
								<td>Member</td>
							</tr>
							<tr>
								<td>9</td>
								<td>A.P. Aarti Bakshi</td>
								<td>Member</td>
							</tr>
							<tr>
								<td>10</td>
								<td>A.P. Pratap Nair</td>
								<td>Member</td>
							</tr>
							<tr>
								<td>11</td>
								<td>A.P. Shubhangi Verulkar</td>
								<td>Member</td>
							</tr>
							<tr>
								<td>12</td>
								<td>A.P. Prasannati Kulkarni</td>
								<td>Member</td>
							</tr>
							<tr>
								<td>13</td>
								<td>A.P. Hasib Shaikh</td>
								<td>Member</td>
							</tr>
						</tbody>
					</table>
				</div>
			</div>
			{{-- 6. Grievance Redressal Committee< --}}
			<div class="row">
				<div class="col-sm-12">
					<h2>6. Grievance Redressal Committee</h2>
					<table class="table table-responsive table-hover table-striped">
						<thead>
							<tr>
								<th width="10%">Sr No</th>
								<th width="60%">Name</th>
								<th width="30%">Designation</th>
							</tr>
						</thead>
						<tbody>
							<tr>
								<td>1</td>
								<td>Prof. Reeta Shaktivel</td>
								<td>Coordinator</td>
							</tr>
							<tr>
								<td>2</td>
								<td>Dr.N.S. Poonalwala</td>
								<td>Ombudsman</td>
							</tr>
							<tr>
								<td>3</td>
								<td>Prof. Pallavi Chopde</td>
								<td>Member</td>
							</tr>
							<tr>
								<td>4</td>
								<td>Prof. Varsha Wangikar</td>
								<td>Member</td>
							</tr>
							<tr>
								<td>5</td>
								<td>Prof. Shrikesh Poojari</td>
								<td>Member</td>
							</tr>
							<tr>
								<td>6</td>
								<td>Mr. Mahesh Dalvi</td>
								<td>Member</td>
							</tr>
							<tr>
								<td>7</td>
								<td>Ms. Pramila B.</td>
								<td>Member</td>
							</tr>
						</tbody>
					</table>
				</div>
			</div>
			{{-- 7. Internal Complaint Committee --}}
			<div class="row">
				<div class="col-sm-12">
					<h2>7. Internal Complaint Committee</h2>
					<table class="table table-responsive table-hover table-striped">
						<thead>
							<tr>
								<th width="10%">Sr No</th>
								<th width="60%">Name</th>
								<th width="30%">Designation</th>
							</tr>
						</thead>
						<tbody>
							<tr>
								<td>1</td>
								<td>Prof. Reeta Shaktivel</td>
								<td>Coordinator</td>
							</tr>
							<tr>
								<td>2</td>
								<td>Prof. Amarja Adgaonkar</td>
								<td>Member</td>
							</tr>
							<tr>
								<td>3</td>
								<td>Prof. Shrikesh Poojari</td>
								<td>Member</td>
							</tr>
							<tr>
								<td>4</td>
								<td>Prof. Uday Prajapati</td>
								<td>Member</td>
							</tr>
							<tr>
								<td>5</td>
								<td>Prof. Amuthavalli Yadav</td>
								<td>Member</td>
							</tr>
							<tr>
								<td>6</td>
								<td>Ms. Priya Naik</td>
								<td>Member</td>
							</tr>
							<tr>
								<td>7</td>
								<td>Ad. Rahul Ranpise</td>
								<td>Member</td>
							</tr>
							<tr>
								<td>8</td>
								<td>Ms. Parita Danole</td>
								<td>Student Representative</td>
							</tr>
						</tbody>
					</table>
				</div>
			</div>
			{{-- 8. Entrepreneurship Development Cell --}}
			<div class="row">
				<div class="col-sm-12">
					<h2>8. Entrepreneurship Development Cell</h2>
					<table class="table table-responsive table-hover table-striped">
						<thead>
							<tr>
								<th width="10%">Sr No</th>
								<th width="60%">Name</th>
								<th width="30%">Designation</th>
							</tr>
						</thead>
						<tbody>
							<tr>
								<td>1</td>
								<td>Prof. Yogesh Karunakar</td>
								<td>Coordinator</td>
							</tr>
						</tbody>
					</table>
				</div>
			</div>
			{{-- 9. Trainning & Placement Cell --}}
			<div class="row">
				<div class="col-sm-12">
					<h2>9. Training & Placement Cell</h2>
					<table class="table table-responsive table-hover table-striped">
						<thead>
							<tr>
								<th width="10%">Sr No</th>
								<th width="60%">Name</th>
								<th width="30%">Designation</th>
							</tr>
						</thead>
						<tbody>
							<tr>
								<td>1</td>
								<td>Dr. Ravi Prakash</td>
								<td>T & P Officer</td>
							</tr>
							<tr>
								<td>2</td>
								<td>Prof. Pratap Nair</td>
								<td>TPC</td>
							</tr>
							<tr>
								<td>3</td>
								<td>Prof. Archana Gupte</td>
								<td>TPC</td>
							</tr>
							<tr>
								<td>4</td>
								<td>Prof. Yamini Patil</td>
								<td>TPC</td>
							</tr>
							<tr>
								<td>5</td>
								<td>Prof. Sheena Unmesh</td>
								<td>Member</td>
							</tr>

							<tr>
								<td>6</td>
								<td>A.P. Shwan L.</td>
								<td>Member</td>
							</tr>
						</tbody>
					</table>
				</div>
			</div>
			{{-- 10. Examination Committee --}}
			<div class="row">
				<div class="col-sm-12">
					<h2>10. Examination Committee</h2>
					<table class="table table-responsive table-hover table-striped">
						<thead>
							<tr>
								<th width="10%">Sr No</th>
								<th width="60%">Name</th>
								<th width="30%">Designation</th>
							</tr>
						</thead>
						<tbody>
							<tr>
								<td>1</td>
								<td>Prof. Reeta Shaktivel</td>
								<td>Exam Cell Incharge</td>
							</tr>
							<tr>
								<td>2</td>
								<td>Prof. Paurnima Vadak</td>
								<td>Assistant Exam Cell Incharge</td>
							</tr>
							<tr>
								<td>3</td>
								<td>Prof.Anupama Chaurasia</td>
								<td>Member</td>
							</tr>
							<tr>
								<td>4</td>
								<td>Prof. Keerti Kharatmol</td>
								<td>Member</td>
							</tr>
							<tr>
								<td>5</td>
								<td>Prof. Shubhangi Verulekar</td>
								<td>Member</td>
							</tr>
							<tr>
								<td>6</td>
								<td>Prof. Neha Khare</td>
								<td>Member</td>
							</tr>
							<tr>
								<td>7</td>
								<td>Prof. Shwan L.</td>
								<td>Member</td>
							</tr>
						</tbody>
					</table>
				</div>
			</div>
			{{-- 11. Sports Committee --}}
			<div class="row">
				<div class="col-sm-12">
					<h2>11. Sports Committee</h2>
					<table class="table table-responsive table-hover table-striped">
						<thead>
							<tr>
								<th width="10%">Sr No</th>
								<th width="60%">Name</th>
								<th width="30%">Designation</th>
							</tr>
						</thead>
						<tbody>
							<tr>
								<td>1</td>
								<td>Prof. Ajay Mishra</td>
								<td>Sports Incharge</td>
							</tr>
							<tr>
								<td>2</td>
								<td>Prof. Tushar Dangat</td>
								<td>Member</td>
							</tr>
							<tr>
								<td>3</td>
								<td>Prof. Pratap Nair</td>
								<td>Member</td>
							</tr>
							<tr>
								<td>4</td>
								<td>Prof. Sumita Gupta</td>
								<td>Member</td>
							</tr>
						</tbody>
					</table>
				</div>
			</div>
			{{-- 12. Career Guidance Committee --}}
			<div class="row">
				<div class="col-sm-12">
					<h2>12. Career Guidance Committee</h2>
					<table class="table table-responsive table-hover table-striped">
						<thead>
							<tr>
								<th width="10%">Sr No</th>
								<th width="60%">Name</th>
								<th width="30%">Designation</th>
							</tr>
						</thead>
						<tbody>
							<tr>
								<td>1</td>
								<td>Dr Puja Pradhan</td>
								<td>Coordinator</td>
							</tr>
							<tr>
								<td>2</td>
								<td>Dr. Arundhati Chakrabarti</td>
								<td>Member</td>
							</tr>
							<tr>
								<td>3</td>
								<td>Dr. Shelley Oberoi</td>
								<td>Member</td>
							</tr>
							<tr>
								<td>4</td>
								<td>Dr. Baban Rindhe</td>
								<td>Member</td>
							</tr>
							<tr>
								<td>5</td>
								<td>Prof. Rajiv Iyer</td>
								<td>Member</td>
							</tr>
							<tr>
								<td>6</td>
								<td>Prof. Yogesh Karunakar</td>
								<td>Member</td>
							</tr>
							<tr>
								<td>7</td>
								<td>Prof. Devika Roy</td>
								<td>Member</td>
							</tr>
							<tr>
								<td>8</td>
								<td>Dr. Ravi Prakash</td>
								<td>Member</td>
							</tr>
							<tr>
								<td>9</td>
								<td>Prof. Pratap Nair</td>
								<td>Member</td>
							</tr>
							<tr>
								<td>10</td>
								<td>Prof. Archana Gupte</td>
								<td>Member</td>
							</tr>
						</tbody>
					</table>
				</div>
			</div>
			{{-- 13. Website Handling Committee --}}
			<div class="row">
				<div class="col-sm-12">
					<h2>13. Website Handling Committee</h2>
					<table class="table table-responsive table-hover table-striped">
						<thead>
							<tr>
								<th width="10%">Sr No</th>
								<th width="60%">Name</th>
								<th width="30%">Designation</th>
							</tr>
						</thead>
						<tbody>
							<tr>
								<td>1</td>
								<td>Prof. Amuthavalli Yadav</td>
								<td>Coordinator</td>
							</tr>
							<tr>
								<td>2</td>
								<td>Prof. Christina Bage</td>
								<td>Member</td>
							</tr>
							<tr>
								<td>3</td>
								<td>Ms. Nayna Khandekar </td>
								<td>Member</td>
							</tr>
						</tbody>
					</table>
				</div>
			</div>
			{{-- 14. Purchase Committee --}}
			<div class="row">
				<div class="col-sm-12">
					<h2>14. Purchase Committee</h2>
					<table class="table table-responsive table-hover table-striped">
						<thead>
							<tr>
								<th width="10%">Sr No</th>
								<th width="60%">Name</th>
								<th width="30%">Designation</th>
							</tr>
						</thead>
						<tbody>
							<tr>
								<td>1</td>
								<td>Dr. Saikiran Khanna</td>
								<td>MD</td>
							</tr>
							<tr>
								<td>2</td>
								<td>Dr.Vilas Nitnaware</td>
								<td>Principal</td>
							</tr>
							<tr>
								<td>3</td>
								<td>Prof. Rajiv Iyer</td>
								<td>HOD</td>
							</tr>
							<tr>
								<td>4</td>
								<td>Dr. Baban Rindhe</td>
								<td>HOD </td>
							</tr>
							<tr>
								<td>5</td>
								<td>Prof. Mandar Ganjapurkar</td>
								<td>HOD</td>
							</tr>
							<tr>
								<td>6</td>
								<td>Prof. Amarja Adgaonkar</td>
								<td>HOD</td>
							</tr>
							<tr>
								<td>7</td>
								<td>Dr. Shelley Oberoi</td>
								<td>HOD</td>
							</tr>
							<tr>
								<td>8</td>
								<td>Prof. Shrikesh Poojari</td>
								<td>HOD</td>
							</tr>
							<tr>
								<td>9</td>
								<td>Mr. Mahesh Dalvi</td>
								<td>Office Superintendent</td>
							</tr>
							<tr>
								<td>10</td>
								<td>Mr. Rohit Kanojia</td>
								<td>Store Incharge</td>
							</tr>
							<tr>
								<td>11</td>
								<td>Ms. Pramila B.</td>
								<td>Accountant</td>
							</tr>
						</tbody>
					</table>
				</div>
			</div>
			{{-- 15. Unfair Means Inquiry Committee --}}
			<div class="row">
				<div class="col-sm-12">
					<h2>15. Unfair Means Inquiry Committee</h2>
					<table class="table table-responsive table-hover table-striped">
						<thead>
							<tr>
								<th width="10%">Sr No</th>
								<th width="60%">Name</th>
								<th width="30%">Designation</th>
							</tr>
						</thead>
						<tbody>
							<tr>
								<td>1</td>
								<td>Dr. Baban Rindhe</td>
								<td>Chairman</td>
							</tr>
							<tr>
								<td>2</td>
								<td>Prof. Bhuma Devi</td>
								<td>Member</td>
							</tr>
							<tr>
								<td>3</td>
								<td>Dr. Shelley Oberoi</td>
								<td>Member</td>
							</tr>
							<tr>
								<td>4</td>
								<td>Prof. Aarti Bakshi</td>
								<td>Member</td>
							</tr>
							<tr>
								<td>5</td>
								<td>Prof. Amarja Adgaonkar</td>
								<td>Member</td>
							</tr>
							<tr>
								<td>6</td>
								<td>Prof.Shirkesh Poojari</td>
								<td> Member </td>
							</tr>
						</tbody>
					</table>
				</div>
			</div>
			{{-- 16. Alumni Committee --}}
			<div class="row">
				<div class="col-sm-12">
					<h2>16. Alumni Committee</h2>
					<table class="table table-responsive table-hover table-striped">
						<thead>
							<tr>
								<th width="10%">Sr No</th>
								<th width="60%">Name</th>
								<th width="30%">Designation</th>
							</tr>
						</thead>
						<tbody>
							<tr>
								<td>1</td>
								<td>Prof. Sonal Balpande</td>
								<td>Coordinator</td>
							</tr>
							<tr>
								<td>2</td>
								<td>Prof. Sheena Unmesh</td>
								<td>Member</td>
							</tr>
							<tr>
								<td>3</td>
								<td>Prof. Manjusha Shelke</td>
								<td>Member</td>
							</tr>
							<tr>
								<td>4</td>
								<td>Prof. Preeti Lodaya</td>
								<td>Member</td>
							</tr>
							<tr>
								<td>5</td>
								<td>Prof. Shawn Langford</td>
								<td>Member</td>
							</tr>
							<tr>
								<td>6</td>
								<td>Prof. Manjusha Shelke</td>
								<td>Member</td>
							</tr>
						</tbody>
					</table>
				</div>
			</div>
			{{-- 17. Marathi Bhasha Wagdmay Mandal - Nakshatra --}}
			<div class="row">
				<div class="col-sm-12">
					<h2>17. Marathi Bhasha Wangdmay Mandal - Nakshatra</h2>
					<table class="table table-responsive table-hover table-striped">
						<thead>
							<tr>
								<th width="10%">Sr No</th>
								<th width="60%">Name</th>
								<th width="30%">Designation</th>
							</tr>
						</thead>
						<tbody>
							<tr>
								<td>1</td>
								<td>Prof. Nilima Patil</td>
								<td>Member</td>
							</tr>
							<tr>
								<td>2</td>
								<td>Prof. Manjusha Shelke</td>
								<td>Member</td>
							</tr>
							<tr>
								<td>3</td>
								<td>Prof. Swati Panvalkar</td>
								<td>Member</td>
							</tr>
						</tbody>
					</table>
				</div>
			</div>
			{{-- 18. Canteen Committee --}}
			<div class="row">
				<div class="col-sm-12">
					<h2>18. Canteen Committee</h2>
					<table class="table table-responsive table-hover table-striped">
						<thead>
							<tr>
								<th width="10%">Sr No</th>
								<th width="60%">Name</th>
								<th width="30%">Designation</th>
							</tr>
						</thead>
						<tbody>
							<tr>
								<td>1</td>
								<td>Prof.Uday Prajapati</td>
								<td>Member</td>
							</tr>
							<tr>
								<td>2</td>
								<td>Prof. Rupinder Kaur</td>
								<td>Member</td>
							</tr>
							<tr>
								<td>3</td>
								<td>Prof. Hasib Shaikh</td>
								<td>Member</td>
							</tr>
						</tbody>
					</table>
				</div>
			</div>
			{{-- 19. NSS Committee --}}
			<div class="row">
				<div class="col-sm-12">
					<h2>19. NSS Committee</h2>
					<table class="table table-responsive table-hover table-striped">
						<thead>
							<tr>
								<th width="10%">Sr No</th>
								<th width="60%">Name</th>
								<th width="30%">Designation</th>
							</tr>
						</thead>
						<tbody>
							<tr>
								<td>1</td>
								<td>Prof. Pratap Nair</td>
								<td>Program Officer & Co-ordinator</td>
							</tr>
							<tr>
								<td>2</td>
								<td>Prof.Geetanjali Kale</td>
								<td>Program Officer</td>
							</tr>
							<tr>
								<td>3</td>
								<td>Prof. Ajay Mishra</td>
								<td>Program Officer</td>
							</tr>
						</tbody>
					</table>
				</div>
			</div>
			{{-- 20. ECO Club --}}
			<div class="row">
				<div class="col-sm-12">
					<h2>20. ECO Club</h2>
					<table class="table table-responsive table-hover table-striped">
						<thead>
							<tr>
								<th width="10%">Sr No</th>
								<th width="60%">Name</th>
								<th width="30%">Designation</th>
							</tr>
						</thead>
						<tbody>
							<tr>
								<td>1</td>
								<td>Prof. Neha Khare</td>
								<td>Coordinator</td>
							</tr>
							<tr>
								<td>2</td>
								<td>Prof. Sandesh Bhingardeve</td>
								<td>Member</td>
							</tr>
							<tr>
								<td>3</td>
								<td>Prof. Sushma Kore</td>
								<td>Member</td>
							</tr>
							<tr>
								<td>4</td>
								<td>Prof. Zubeda Khan</td>
								<td>Member</td>
							</tr>
						</tbody>
					</table>
				</div>
			</div>
			{{-- 21. WDC / Gender Equity Committee --}}
			<div class="row">
				<div class="col-sm-12">
					<h2>21. WDC / Gender Equity Committee</h2>
					<table class="table table-responsive table-hover table-striped">
						<thead>
							<tr>
								<th width="10%">Sr No</th>
								<th width="60%">Name</th>
								<th width="30%">Designation</th>
							</tr>
						</thead>
						<tbody>
							<tr>
								<td>1</td>
								<td>Prof. Pallavi Chopade</td>
								<td>Coordinator</td>
							</tr>
							<tr>
								<td>2</td>
								<td>Prof. Vaishali Yeole</td>
								<td>Member</td>
							</tr>
							<tr>
								<td>3</td>
								<td>Prof. Manasi Choche</td>
								<td>Member</td>
							</tr>
							<tr>
								<td>4</td>
								<td>Prof. Deepak Khabale</td>
								<td>Member</td>
							</tr>
							<tr>
								<td>5</td>
								<td>Prof. Anupama Chaurasia</td>
								<td>Member</td>
							</tr>
							<tr>
								<td>6</td>
								<td>Prof. Shawn Langford</td>
								<td>Member</td>
							</tr>
						</tbody>
					</table>
				</div>
			</div>
			{{-- 22. Rotaract --}}
			<div class="row">
				<div class="col-sm-12">
					<h2>22. Rotaract</h2>
					<table class="table table-responsive table-hover table-striped">
						<thead>
							<tr>
								<th width="10%">Sr No</th>
								<th width="60%">Name</th>
								<th width="30%">Designation</th>
							</tr>
						</thead>
						<tbody>
							<tr>
								<td>1</td>
								<td>Prof. Sheena Unmesh</td>
								<td>Coordinator</td>
							</tr>
							<tr>
								<td>2</td>
								<td>Prof. Pallavi Chopade</td>
								<td>Member</td>
							</tr>
							<tr>
								<td>3</td>
								<td>Prof. Shailaja Mohite</td>
								<td>Member</td>
							</tr>
						</tbody>
					</table>
				</div>
			</div>
			{{-- 23. YRC --}}
			<div class="row">
				<div class="col-sm-12">
					<h2>23. YRC</h2>
					<table class="table table-responsive table-hover table-striped">
						<thead>
							<tr>
								<th width="10%">Sr No</th>
								<th width="60%">Name</th>
								<th width="30%">Designation</th>
							</tr>
						</thead>
						<tbody>
							<tr>
								<td>1</td>
								<td>Prof. Geetanjali Kale</td>
								<td>Coordinator</td>
							</tr>
							<tr>
								<td>2</td>
								<td>Prof.Sana Haji</td>
								<td>Member</td>
							</tr>

						</tbody>
					</table>
				</div>
			</div>
		</div>
	</div>
</div>
@endsection
@section('post')
@endsection
